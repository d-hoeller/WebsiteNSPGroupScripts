...
class web_site_publications:
    ...
    def copy_base_site(self, source, dest):
        with open(dest, "w") as f_out:
            # f_out.write("<!--\nDO NOT CHANGE, THIS FILE IS AUTOMATICALLY GENERATED, CHANGES WILL BE OVERWRITTEN\n-->\n\n")
...
if __name__ == '__main__':
    ...
    #
    # write personal sites
    #
    pers_sites = [('Daniel Höller', 'hoeller'),
                  ('Marcel Schubert', 'schubert'),
                  ('Jonas Kück', 'krueck')]

    for full, short in pers_sites:
        source = os.path.join(base_path, "content", short + ".base")
        dest =  os.path.join(base_path, "content", short + ".md")
        pub.copy_base_site(source, dest)
        filter = {'author[or]editor': full}
        pub.append_bib_entries(dest, filter)
